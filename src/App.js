...
class App extends Component {
	...
	makeImg = (img) => {
		axios
			...
			.then((img) => this.setState({ imgs: [ ...this.state.imgs, img ] }));
	};
	...
	setImgPosition = (e) => {
		...
		//+ 58 worked
		const diff = e.clientY - this.spanRef.current.parentElement.offsetTop;
		const diff2 = e.clientX - this.spanRef.current.parentElement.offsetLeft;
		const top = e.clientY - e.target.offsetTop + diff; //rougly distance from h3 to top of img
		...
		console.log(
			'target offsetTop vs. spanRef parent offsetTop: ',
			e.target.offsetLeft,
			this.spanRef.current.parentElement.offsetLeft
		);
		...
		console.log(`clientY ${e.clientY} - target.offsetTop ${e.target.offsetTop}`);
		console.log(`clientX ${e.clientX} - target.offsetLeft ${e.target.offsetLeft}`);
	};
